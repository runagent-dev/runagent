...
```javascript Becomes JavaScript Streams
import { RunAgentClient } from 'runagent';

const client = new RunAgentClient({
    agentId: 'your-agent-id',
    entrypointTag: 'generate_report_stream',
    local: true
});
...
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let client = RunAgentClient::new("your-agent-id", "generate_report_stream", true).await?;
...
- [Quick Start Guide](/get-started/quickstart) - Deploy your first AI agent in under 5 minutes and see the magic happen
- [Core Concepts](/get-started/core-concepts) - Master the fundamental concepts that make RunAgent powerful

- [Framework Overview](/frameworks/overview) - Deploy agents built with LangGraph, CrewAI, Letta, or any AI framework

- [Python SDK](/sdk/python/getting-started) - Start using RunAgent SDKs in Python with complete examples

- [CLI Overview](/cli/overview) - Master the RunAgent command-line interface for project management

- [Configuration File](/configuration/config-file) - Understanding runagent.config.json and entrypoint configuration