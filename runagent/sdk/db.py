...
class DBService:
    ...
    def _check_enhanced_limits_with_fallback(self) -> Dict:
        ...
        if (self._limits_cache and self._cache_expiry and datetime.now() < self._cache_expiry):
        ...
        try:
            ...
            if limits_data.get('success'):
                ...
                if enhanced:
                    console.print(f"🔑 [green]Enhanced limits active: {max_agents} agents[/green]")
                else:
                    console.print(f"🔑 [yellow]Using default limits: {max_agents} agents[/yellow]")
                ...
                console.print(f"[yellow]⚠️ API limit check failed: {error_msg} - using default limits[/yellow]")
                return result
            ...
            console.print(f"[red]❌ Error checking enhanced limits: {str(e)} - using default limits[/red]")
            return result
    ...
    def add_agent(self, agent_id: str, agent_path: str, host: str = 'localhost', 
                  port: int = 8000, framework: str = None, status: str = 'deployed') -> Dict:
        """
        ...
        with self.db_manager.get_session() as session:
            try:
                ...
                if current_count < default_limit:
                    console.print(f"🟢 Adding agent within default limits ({current_count + 1}/{default_limit})")
                ...
                console.print(f"🟡 At default limit ({current_count}/{default_limit}) - checking for enhanced limits...")
                ...
                if current_count >= max_capacity:
                    oldest_agent = session.query(Agent).order_by(Agent.deployed_at).first()
                    ...
                    if not self.rest_client:
                        ...
                        suggestion = f'Consider replacing the oldest agent: {oldest_agent.agent_id}' if oldest_agent else 'Database cleanup needed'
                    ...
                    return {
                        ...
                        'oldest_agent': {
                            ...
                            'deployed_at': oldest_agent.deployed_at.isoformat() if oldest_agent.deployed_at else None
                ...
                console.print(f"🟢 Agent added with {limit_source} limits ({current_count + 1}/{max_capacity})")
                ...
                return {
                    ...
                    'current_count': current_count + 1,
                    'max_allowed': max_capacity,
                    'remaining_slots': max_capacity - (current_count + 1) if max_capacity != 999 else 'unlimited',
                    'limit_info': limit_info,
    ...
    def replace_agent(self, old_agent_id: str, new_agent_id: str, agent_path: str,
                     host: str = 'localhost', port: int = 8000, framework: str = None, 
                     ) -> Dict:
        """
        ...
        with self.db_manager.get_session() as session:
            try:
                ...
                old_agent = session.query(Agent).filter(Agent.agent_id == old_agent_id).first()
                if not old_agent:
                    return {
                        'success': False,
                ...
                existing_new_agent = session.query(Agent).filter(Agent.agent_id == new_agent_id).first()
    ...
    def delete_agent(self, agent_id: str) -> bool:
        ...
        """
        console.print(f"⚠️ Agent deletion is disabled. Agent {agent_id} cannot be deleted from database.")
        ...
        with self.db_manager.get_session() as session:
            try:
                ...
                return {
                    ...
                    'default_limit': default_limit,
                    'remaining_slots': max(0, max_capacity - current_count) if max_capacity != 999 else 'unlimited',
                    ...
                    'oldest_agent': {
                        ...
                        'deployed_at': agents[0].deployed_at.isoformat() if agents[0].deployed_at else None
                    } if agents else None,
                    'newest_agent': {
                        'agent_id': agents[-1].agent_id,
                        'deployed_at': agents[-1].deployed_at.isoformat() if agents[-1].deployed_at else None
                    } if agents else None,
                    'rest_client_configured': self.rest_client is not None
                }
            except Exception as e:
    ...
    def update_api_key(self, api_key: str) -> Dict:
        ...
        """
        console.print("[yellow]⚠️ update_api_key is deprecated. Please configure API key in RestClient.[/yellow]")
        return {
            'success': False,
    ...
    def clear_api_key(self) -> Dict:
        ...
        console.print("[yellow]⚠️ clear_api_key is deprecated. Please configure API key in RestClient.[/yellow]")
        return {
            'success': False,
            'error': 'API key management moved to RestClient. Please use RestClient for API operations.',
            'deprecated': True
    ...
    def get_agent_stats(self, agent_id: str) -> Dict:
        ...
        with self.db_manager.get_session() as session:
            try:
                ...
                ).order_by(desc(AgentRun.started_at)).limit(10).all()
                
                # Calculate statistics
                total_runs = agent_data['run_count']
                success_rate = (agent_data['success_count'] / total_runs * 100) if total_runs > 0 else 0
                
                avg_execution_time = None
                if recent_runs:
                    execution_times = [run.execution_time for run in recent_runs if run.execution_time is not None]
                    if execution_times:
    ...
    def cleanup_old_runs(self, days_old: int = 30) -> int:
        ...
        with self.db_manager.get_session() as session:
            try:
                ...
                deleted_count = session.query(AgentRun).filter(
        ...
        with self.db_manager.get_session() as session:
            try:
                ...
                status_results = session.query(
                    Agent.status, func.count(Agent.agent_id)
                ...
                # Get database size
                db_size = self.db_manager.db_path.stat().st_size if self.db_manager.db_path.exists() else 0
