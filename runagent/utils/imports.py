...
class DependencyAnalyzer:
    ...
    def analyze_file(self, file_path: Path, package_root: Path) -> Dict[str, Set[str]]:
        ...
        try:
            ...
            return {'imports': set(), 'from_imports': set(), 'relative_imports': set(), 
                   'star_imports': set(), 'conditional_imports': set()}
    ...
    def _process_from_import(self, node: ast.ImportFrom, current_module: str, dependencies: Dict[str, Set[str]]):
    ...
    def _resolve_relative_import(self, node: ast.ImportFrom, current_module: str) -> str:
    ...
    def _process_conditional_imports(self, node: ast.Try, dependencies: Dict[str, Set[str]]):
...
class PackageImporter:
    ...
    def resolve_import(self, entrypoint_filepath: str, function_name: str) -> Callable:
        ...
        try:
            ...
            if not hasattr(entry_module, function_name):
                raise AttributeError(f"Function '{function_name}' not found in {entrypoint_filepath}")
            
            target_function = getattr(entry_module, function_name)
            
            if not callable(target_function):
                raise TypeError(f"'{function_name}' in {entrypoint_filepath} is not callable")
            ...
            if self.verbose:
                ...
                if hasattr(target_function, '__doc__') and target_function.__doc__:
                    logger.info(f"   Doc: {target_function.__doc__.strip().split('.')[0]}")
    ...
    def _find_project_root(self, entrypoint_path: Path) -> Path:
        ...
        project_indicators = ["__init__.py", "setup.py", "pyproject.toml", "requirements.txt", ".git"]
    ...
    def _load_package_structure(self):
        ...
        for module_name in load_order:
            if module_name not in self.loaded_modules and module_name not in self.failed_modules:
    ...
    def _analyze_package_structure(self):
        ...
        for py_file in py_files:
            ...
            dependencies = self.dependency_analyzer.analyze_file(py_file, self.package_root)
    ...
    def get_load_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": f"{(loaded_count / total_attempted * 100):.1f}%" if total_attempted > 0 else "100%",
