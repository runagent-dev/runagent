...
from runagent.constants import ENV_RUNAGENT_LOGGING_LEVEL
from runagent.exceptions import ApiKeyNotProvidedError


_DEFAULT_FORMAT = "[%(asctime)s][%(levelname)s] %(message)s"
_DEFAULT_LOGGER_NAME = "runagent"

_LOG_VERBOSITY = int(os.environ.get("RUNAGENT_LOG_VERBOSITY", 0))
...
"""Global logger object for runagent."""

_logger_wrapper: t.Optional["_VerbosityWrapper"] = None
"""Global logger object wrapped with verbosity setting for runagent."""
...
def _parse_log_level_from_env(default: int) -> int:
    ...
    level = os.environ.get(ENV_RUNAGENT_LOGGING_LEVEL)
    if level is None:
        return default
...
class LogIngester:
    def __init__(self) -> None:
        from runagent import RunAgent  # pylint: disable=import-outside-toplevel

        try:
            self._client = RunAgent.get_latest()
