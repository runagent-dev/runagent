...
interface NodeModules {
  ...
  function isNodeEnvironmentReady(): boolean {
    return !!(nodeModules.Database && nodeModules.path && nodeModules.os && nodeModules.fs);
  }
  ...
  export class RunAgentRegistry {
    ...
    constructor(cacheDir?: string) {
      if (!isNode) {
        throw new Error('RunAgentRegistry is only available in Node.js environment');
      }
    ...
    async initialize(): Promise<void> {
      ...
      if (!dependenciesLoaded || !isNodeEnvironmentReady()) {
        throw new Error('Required dependencies not available. Install with: npm install better-sqlite3');
      }
  
      // Set default cache dir if not provided
      if (!this.dbPath) {
        this.dbPath = nodeModules.path.join(nodeModules.os.homedir(), '.runagent');
      }
  
      const dbFile = nodeModules.path.join(this.dbPath, 'runagent.db');
    ...
    lookupAgent(agentId: string): { host: string; port: number } {
      ...
      if (agent.status !== 'deployed') {
        throw new Error(`Agent ${agentId} is not deployed (status: ${agent.status})`);
      }
