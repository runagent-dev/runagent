...
def test_mock_agent():
    ...
    email_prompt = [
        {"role": "user", "content": "Write a professional email with sender: Alice Johnson, recipient: Mr. Daniel Smith, subject: Request for Meeting Next Week"}
    ]
    ...
    print("\n=== Testing Streaming ===")
    for chunk in client.create(model="gpt-4", messages=email_prompt, stream=True):
    ...
    response = client.create(messages=code_prompt)
    print(response.content)
    
    print("\n=== Testing Analysis ===")
    analysis_prompt = [
        {"role": "user", "content": "Analyze the benefits of remote work for software teams"}
    ]
    
    response = client.create(messages=analysis_prompt)
    print(response.content)
