...
class TemplateFactory:
    ...
    def get_template(cls, framework: str, level: str = "basic") -> BaseTemplate:
        ...
        template_instance = cls._enhance_template_with_logging(
            template_instance, framework
        )

        return template_instance

    @classmethod
    def _enhance_template_with_logging(
        cls, template: BaseTemplate, framework: str
    ) -> BaseTemplate:
        """
...
"""
                ...
                if run_start_idx >= 0:
                    ...
                    if run_body_start >= 0:
                        enhanced_code = (
                            enhanced_code[: run_body_start + 1]
                            + start_time_code
                            + enhanced_code[run_body_start + 1 :]
                        )
                    ...
                    if try_start_idx >= 0:
                        ...
                        return_idx = enhanced_code.find(
                            "        return ", try_start_idx
                        )
                        if return_idx >= 0:
                            # Get the return variable name
                            return_line = enhanced_code[
                                return_idx : enhanced_code.find("\n", return_idx)
                            ]
                        ...
                        except_start_idx = enhanced_code.find(
                            "    except", try_start_idx
                        )
                        if except_start_idx >= 0:
                            # Find the return in the except block
                            except_return_idx = enhanced_code.find(
                                "        return", except_start_idx
                            )
    ...
    def register_template(
        cls, framework: str, level: str, template_class: Type[BaseTemplate]
    ):
