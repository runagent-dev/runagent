#!/bin/bash
# Standalone Linux build script for RunAgent with Nuitka
# Usage: ./build_scripts/build-linux-standalone.sh
# Or from root: bash build_scripts/build-linux-standalone.sh

set -e  # Exit on error

echo "🐧 RunAgent Linux Build Script (Standalone)"
echo "==========================================="
echo ""
...
if ! command -v python3 &> /dev/null; then
    ...
    echo "RHEL/Fedora: sudo dnf install python3 python3-devel"
...
if ! command -v gcc &> /dev/null; then
    ...
    if command -v apt-get &> /dev/null; then
        ...
        sudo dnf install -y gcc gcc-c++ python3-devel ccache patchelf
    elif command -v yum &> /dev/null; then
        sudo yum install -y gcc gcc-c++ python3-devel ccache patchelf
    else
...
echo ""
echo "🖥️  Building for Linux-$ARCH_NAME"
echo "⏳ This will take 5-10 minutes on first build..."
echo ""

BUILD_DIR="dist/linux-${ARCH_NAME}"
...
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
exec "$SCRIPT_DIR/runagent_entry.dist/runagent" "$@"
EOF
    chmod +x "$WRAPPER"
    ...
    cd "$BUILD_DIR"
    tar -czf "../runagent-linux-${ARCH_NAME}.tar.gz" runagent_entry.dist/
    cd - > /dev/null
    
    TARBALL_SIZE=$(ls -lh "dist/runagent-linux-${ARCH_NAME}.tar.gz" | awk '{print $5}')
    echo "✅ Release archive created: dist/runagent-linux-${ARCH_NAME}.tar.gz ($TARBALL_SIZE)"
    echo ""
